...
watch(() => nftMarketplace.getEarnings, function() {
...
<div class="column items-center">
  <q-card v-if="connection.currentAddress" :style="{height: '200px', width: '500px'}" class="q-mt-xl">
    <div class="column items-center q-mt-lg justify-center">
        <div class="text-subtitle1">Your earnings so far</div>
        <div class="q-mt-md text-h3 text-positive">{{ earnings }} ETH</div>
        <q-btn :disable="earnings === 0" @click="withdraw" class="q-mt-md" color="primary" label="GET THEM!" />
    </div>
  ...
  <q-card v-else :style="{height: '200px', width: '500px'}" class="q-mt-xl">
    <div class="column items-center q-mt-lg justify-center">
        ...
        <q-btn @click="connection.connect()" class="q-mt-md" color="primary" label="CONNECT" />
    </div>
  </q-card>
</div>
</template>
